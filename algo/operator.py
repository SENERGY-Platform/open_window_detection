...
class Operator(util.OperatorBase):
    ...
    def run(self, data, selector = None):
        ...
        if end_mean < front_mean - 2*front_std and front_mean - end_mean > 2 and self.sliding_window[-1]["value"] < self.sliding_window[-2]["value"]:
            ...
            if self.window_open and self.sliding_window[-1]["value"] - self.unsusual_drop_detections[-1]["value"] > 1:
                self.window_open = False
                ...
                print("Window closed!")
        return {"window_open": self.window_open, "timestamp": str(current_timestamp)}
